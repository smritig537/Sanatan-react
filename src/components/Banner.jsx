...
const Banner = () => {
    ...
    return (
        <div id="default-carousel" className="relative w-full" data-carousel="slide">
            <div className="relative h-80 md:h-96 overflow-hidden"> {/* Increased height */}
                {slides.map((slide, index) => (
                    <div
                        key={index}
                        className={`absolute w-full transition-opacity duration-700 ease-in-out ${index === currentIndex ? 'opacity-100' : 'opacity-0'}`} // Fade effect
                        data-carousel-item
                    >
                        <img
                            src={slide.url}
                            className="w-full h-full object-contain" // Changed to object-cover for better fitting
                            alt={slide.alt}
                        />
                    </div>
                ))}
